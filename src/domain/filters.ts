...
export function setFieldValueFilter(filters: ReportFilters, fieldId: string, values: string[]) {
...
export function addFieldValueFilter(filters: ReportFilters, fieldId: string, value: string): ReportFilters {
  const newValues = union(filters.fieldValues[fieldId], [value]);
  return setFieldValueFilter(filters, fieldId, newValues);
}

export function removeFieldValueFilter(filters: ReportFilters, fieldId: string, value: string): ReportFilters {
  const newValues = filters.fieldValues[fieldId]?.filter(val => val !== value);
  return setFieldValueFilter(filters, fieldId, newValues ?? []);
}

export function toggleFieldValueFilter(filters: ReportFilters, fieldId: string, value: string): ReportFilters {
  return filters.fieldValues[fieldId]?.includes(fieldId) ? removeFieldValueFilter(filters, fieldId, value) : addFieldValueFilter(filters, fieldId, value);
}

export function hasFilters(filters: ReportFilters): boolean {
  const { playerCount, expansions, fieldValues } = filters;
  return playerCount != null || expansions.length > 0 || Object.keys(fieldValues).some(
    fieldId => fieldValues[fieldId]?.length > 0,
  );
}
...
export function applyPlayFilters(allPlays: Play[], filters: ReportFilters) {
  ...
  if (playerCount != null) {
    filteredPlays = filteredPlays.filter(play => play.players.length === playerCount);
  }
  // Filter by dimension values
  filteredPlays = Object.keys(fieldValues).reduce(
    (plays, fieldId) => {
      const values = fieldValues[fieldId];
      return plays.filter(
        play => values.every(value => play.hasMiscFieldValue(fieldId, value)),
      );
    },
    filteredPlays,
  );
