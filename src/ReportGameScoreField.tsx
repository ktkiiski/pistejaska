...
function getScoreFieldStatistics(
  game: Game,
  plays: Play[]
): Record<string, ScoreFieldStatistics> {
  ...
  plays.forEach((play) => {
    ...
    fields.forEach(({ field }) => {
      ...
      const scoreRankings = rankScores(
        play.rankings.map((ranking) => ({
          ranking,
      ...
      scoreRankings.forEach((scoreRanking) => {
        ...
        if (normalizedPosition != null && ranking.normalizedPosition != null) {
          ...
          fieldStats.normalizedRankingPositions.push(
            ranking.normalizedPosition
          );
        }
        if (ranking.position === 1) {
          fieldStats.winnerScoreCount += 1;
          fieldStats.winnerScoreSum += score;
          fieldStats.winnerAverageScore =
            fieldStats.winnerScoreSum / fieldStats.winnerScoreCount;
        }
      });
    });
  });
  ...
  Object.values(results).forEach((stats) => {
    const coefficient = calculatePearsonCorrelation(
      stats.normalizedRankingPositions,
      stats.normalizedScorePositions
    );
...
function GameScoreFieldReport(props: { game: Game; plays: Play[] }) {
  ...
  const rows = scoreFieldStats.map(
    ({ field, winnerAverageScore, rankingCorrelation }) => [
      {
        value: field.name,
      },
      {
